 ...
 * 
 * 
 * Wordt (momenteel) niet gebruikt door ons team (groen)
 * 
 * 
 * 
 * 
...
public class PollMonitor {
	...
	public static void main(String[] main) {
		try {
			...
			// This task is scheduled by a timer to periodically get new messages
			...
			TimerTask monitorTask = new TimerTask() {
				...
				public void run() {
					try {
						...
						while (running) {
							boolean noAck = false;
							// get the messages in our queue and do not acknowledge the message
							// until we processed it
							GetResponse response = channel.basicGet(queue.getQueue(), noAck);
							if (response == null) {
								running = false;
							} else {
								...
								// response.getProps().getTimestamp() contains the timestamp
